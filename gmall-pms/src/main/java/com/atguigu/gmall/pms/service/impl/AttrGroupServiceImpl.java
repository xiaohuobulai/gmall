...
public class AttrGroupServiceImpl extends ServiceImpl<AttrGroupMapper, AttrGroupEntity> implements AttrGroupService {
    ...
    @Override
    public List<GroupVo> queryAttrsByCid(Long catId) {
        ...
        List<GroupVo> groupVos = attrGroupEntities.stream().map(AttrGroupEntity -> {
            ...
            List<AttrEntity> attrEntities = this.attrMapper.selectList(new QueryWrapper<AttrEntity>().eq("group_id", groupVo.getId()).eq("type", 1));
